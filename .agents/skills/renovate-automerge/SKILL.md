...
- `aqua.yaml` だけを変更する patch/minor update
  ...
  - `golang/go` は patch/minor まで許可する。ただし Go の release notes に既存コード、module resolution、race detector、cross compile、GoReleaser build へ影響する注意点がある場合はマージしない。
